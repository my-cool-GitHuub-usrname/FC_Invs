...
export class InventoryWindow extends CanvasWindow {
  ...
  onCanvasMouseDown (x, y, secondary) {
    ...
    if (this._lastClick) {
      ...
      if (dist < 0.5 && timeDelta < 200 && !this._lastClick.double) {
        double = true
      }
    }
  ...
  renderOverlays () {
    ...
    if (this._tooltipPath) {
      this.drawImage({path: this._tooltipPath}, x + 7, y - 16)
      //this.drawText({ bg: 1, value: this._tooltipPath, style: 'lightblue', fontStyle: 'normal normal 8px sans-serif', stroke: 'skyblue' }, x + 12, y)
    }
  ...
  renderLayout (obj, xoff = this.xoff, yoff = this.yoff) {
    for (const key in obj) {
      ...
      if (val.draw) { // Custom draw function
        ...
        let i = 0
        // Some defaults
        val.width ??= 1; val.height ??= 1; val.size ??= 16; val.margin ??= 2; val.padding ??= 1
        if (val.ho) { // Scrollbar triggered horizontal offset
          i = val.ho * val.width
        }
        for (let _y = 0; _y < val.height; _y++) {
          for (let _x = 0; _x < val.width; _x++) { //i changed some of the padding calculations to have less empty space on itemgrid gridlines
            const bb = [val.x + (_x * val.size) + xoff + (_x * val.margin), val.y + (_y * val.size) + yoff + (_y * val.margin), val.size, val.size]
            // this.drawBox(bb)
  ...
  onItemEvent (id, type, pos, data) {
    if (type === 'hover') {
      const [cont, i] = data
      this._tooltipPath = this[cont][i]?.tooltipImg
