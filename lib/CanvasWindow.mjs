...
export class CanvasWindow extends SimpleEmitter {
  ...
  drawImage (obj, dx, dy, slice, scale) {
    ...
    scale = scale || 1
    if (obj.path && obj.path.includes("icons/")) {
      this.drawCtx.drawImage(img, dx, dy, (img.width * 0.25), (img.height * 0.25))
    } else if (slice) {                              // generally true for most gui images
      const w = slice[2]; const h = slice[3]
      this.drawCtx.drawImage(img, slice[0], slice[1], slice[2], slice[3], dx, dy, w * scale, h * scale)
    } else {                                         //true for non-icons, non-gui images* (a few odd gui elements, as seen in creativemenu, reach this point)
  ...
  drawBox (aroundBB) {
    this.drawCtx.fillStyle = 'rgba(255, 255, 255, 0.46)' // should be .38 before item rendering and another .12 after item rendering, but i dont feel like it
